...
def validate_header(df, name, affil):
    ...
    if affil not in affils:
        ...
        print('Please enter a validate affiliation')
    ...
    # if length lesss than zero, then player does not exists
    if len(find_player[find_player['Name'] == name]) == 0:
        
        print('Please enter a valid name')
        print('Player may exist but you may have entered the wrong affiliation')
        
        return False
       
    return True

# Main Function Driver - JaeEun, Navigator - Bryan
# Run the program
# Continually loop and ask user for info until user types "no"
if __name__ ==  "__main__":
    
    #test =  load_files()
    #print('NBA' in (test[0]['Affliation'].values))
    ...
    while True:
        ...
        #check if name and affiliation are valid
        if validate_header(load_files(), get_player_name,  get_player_affil):
